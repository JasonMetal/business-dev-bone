...
var (
	unknownCoder defaultCoder = defaultCoder{1, http.StatusInternalServerError, "An internal server error occurred", "https://example.com/doc/main"}
	commonCoder  int
)
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is reserved by `business-dev-bone/pkg/errors` as unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved by 'business-dev-bone/pkg/errors' as ErrUnknown error code")
	}
...
func ParseCoder(err error) Coder {
	...
	if v, ok := err.(*withCode); ok {
		if v.code == commonCoder && v.err.Error() != "" {
			return &defaultCoder{
				C:    v.code,
				HTTP: http.StatusOK,
				Ext:  v.err.Error(),
