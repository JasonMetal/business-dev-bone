...
import (
	...
	"business-dev-bone/pkg/component-base/log/klog"
)
...
func SetStd(name string, logger *zap.Logger) {
	mu.Lock()
	defer mu.Unlock()
	l := &zapLogger{
		zapLogger: logger.Named(name),
		infoLogger: infoLogger{
			log:   logger,
			level: zap.InfoLevel,
		},
	}
	klog.InitLogger(logger)
...
func GetWriter() io.Writer {
	return NewLogWriter(std)
}

func GetGoRequestLogger() *gorequestLogger {
	return NewGoRequestLogger(std)
}
...
// StdInfoLoggerWithTraceID returns a logger of standard library which writes to supplied zap
// logger at info level and adds traceID to the log.
func StdInfoLoggerWithTraceID(ctx context.Context) *log.Logger {
	if std == nil {
		return nil
	}

	l, err := zap.NewStdLogAt(std.L(ctx).zapLogger, zapcore.InfoLevel)
	if err != nil {
		return nil
	}
	return l
...
// Log implement aws-sdk-go/aws.Logger interface
...
func getKeyFromCtx(ctx context.Context, key string) any {
