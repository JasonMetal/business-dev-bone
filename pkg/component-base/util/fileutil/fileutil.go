...
// FileType uses mimetype package to determine the given file path's type.
...
// CheckPathIsDot check path is . or ./
func CheckPathIsDot(path string) bool {
...
// ReadFileInfos recursively reads all files in a given directory
func ReadFileInfos(dir string) ([]FileInfo, error) {
	var files []FileInfo
	...
	err := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		// only read file
		if !info.IsDir() {
			digest, err := calculateMD5(path)
			if err != nil {
				return err
			}

			contentType, err := FileType(path)
			if err != nil {
				return err
			}
